...
public class studentDAO implements CrudRequests<Student, StudentBase> {
    ...
    public List<Student> findByCriteria(List<Criteria> criteriaList,int page, int size){
    List<Student> students = new ArrayList<>();
        StringBuilder query = new StringBuilder("SELECT * FROM student WHERE 1=1");  // Base de requête
        List<Object> params = new ArrayList<>();


        for (Criteria criteria : criteriaList) {
            query.append(" AND ").append(criteria.getField()).append(" ").append(criteria.getOperator()).append(" ?");
